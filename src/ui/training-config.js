 ...
 * @param {number} cursor        – which row is selected (0=room, 1=enemies, 2=count, 3=START)
 * @param {number} roomIndex     – -1 = training room, 0..N = game rooms
 * @param {string} roomName      – display name
 * @param {string} enemyLabel    – e.g. "All Types", "Basic", "Shooter"
 * @param {number} enemyCount    – 1..10
 * @param {boolean} fromGame     – true if player pressed T mid-game (show "Back" instead of "Menu")
 */
export function renderTrainingConfig(ctx, cursor, roomIndex, roomName, enemyLabel, enemyCount, fromGame) {
    // ── Background ──
    ...
    const rowYs = [startY, startY + 70, startY + 172];
    ...
    // ── START button ──
    const btnY = rowYs[2] + boxH + 20;
    const btnSelected = cursor === 3;
    const btnW = 200;
    const btnH = 44;
